...
public class Server {
    ...
    private byte[] constructResponse(String requestContent, byte[] requestResult) {
        ...
        StringBuilder responseHeaders = new StringBuilder().append("HTTP/1.1 200 OK").append(System.lineSeparator())
        ...
        byte[] fullRespose = new byte[headersAsBytes.length + requestResult.length];

        for (int i = 0; i < headersAsBytes.length; i++) {
            fullRespose[i] = headersAsBytes[i];
        }

        for (int i = 0; i < requestResult.length; i++) {
            fullRespose[headersAsBytes.length+i] = requestResult[i];
        }
        return fullRespose;
    ...
    private byte[] get(String requestResource) {
        ...
        try {
            fileByteData = Files.readAllBytes(Paths.get("C:\\Users\\345\\Documents\\Java WEB Development\\03. HTTP Protocol\\Server - Demo\\src\\resources\\" + requestResource));
